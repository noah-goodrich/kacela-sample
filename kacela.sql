...
CREATE PROCEDURE sp_belongs_to (IN schemaName VARCHAR(100), IN tableName VARCHAR(100))
    BEGIN
        SELECT TABLE_NAME as keyTable, COLUMN_NAME AS keyColumn, REFERENCED_TABLE_NAME AS refTable, REFERENCED_COLUMN_NAME AS refColumn, CONSTRAINT_NAME AS constraintName
...
CREATE PROCEDURE sp_has_many (IN schemaName VARCHAR(100), IN tableName VARCHAR(100))
    BEGIN
        SELECT REFERENCED_TABLE_NAME AS keyTable, REFERENCED_COLUMN_NAME AS keyColumn, TABLE_NAME AS refTable, COLUMN_NAME AS refColumn, CONSTRAINT_NAME AS constraintName
        FROM INFORMATION_SCHEMA.key_column_usage
        WHERE TABLE_SCHEMA = schemaName
